...
// 1. 멘토를 추가하면 State가 변경되기 때문에 AppMentorsButton(부모) 함수 컴포넌트가 다시 호출된다.
// 2. AppMentorsButton(부모)가 다시 호출되면 handleAdd, handleUpdate, handleDelete 변수에 새로운 함수 객체 참조값이 할당된다.
...
// 5. 그래서 Button(자식) 컴포넌트도 다시 호출된다. console.log('Button', text, 're-rendering 😁'); 자식 컴포넌트도 리렌더링 발생
// 즉, AppMentorsButton(부모) 컴포넌트에서 상태가 변경될 때마다 자식 컴포넌트들은 다 리렌더링이 된다.
// 하지만 리액트는 Virtual DOM을 가지고 있기 때문에 실제로 브라우저 DOM 요소에는 바뀐 부분만 업데이트 해줌!!
// 하지만 Button 컴포넌트는 수많은 자식 컴포넌트도 가지고 있지 않고, 무거운 작업을 하고 있지 않고 있기 때문에 그렇게 성능에 대한 걱정을 할 필요는 없다.
...
const Button = memo(({ text, onClick }) => {
  ...
  // 리액트가 아무리 Virtual DOM을 쓴다고 해도 실제로 계산하는 코드가 많고 속도가 오래 걸리는 일들을 수행한다면
  // Virtual DOM을 만드는데 까지도 시간이 오래걸리기 때문에 어떤거를 업데이트 해야되는지 결정하는데까지 오랜시간이 걸리기 때문에
  // 우리 어플리케이션의 성능에 큰 영향을 줄 수 있음
